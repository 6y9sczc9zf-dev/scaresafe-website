...
export async function GET() {
  ...
  const [moviesR, candidatesR, proposalsR, reportsR, contributorsR, usersR] = await Promise.all([
    ...
    supabaseRequest("/rest/v1/verification_candidates?select=id,movie_id,candidate_type,timestamp_ms,intensity,description,category,verification_state,upvotes,downvotes,created_at,updated_at&candidate_type=eq.jumpscare&order=timestamp_ms.asc&limit=1000", admin.token),
