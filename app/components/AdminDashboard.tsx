...
type Candidate = { id: string; movie_id: number; candidate_type: string; timestamp_ms: number; intensity?: string; description?: string; category?: string; verification_state: string; upvotes: number; downvotes: number; submitter_name?: string; created_at: string; updated_at: string };
type Proposal = { id: string; movie_id: number; proposal_type: string; proposed_timestamp_ms: number; proposed_intensity?: string; details?: string; state: string; positive_validations: number; negative_validations: number; submitter_name?: string; created_at: string };
...
export default function AdminDashboard({ email }: { email: string }) {
  ...
  async function removeCandidate(candidate: Candidate) {
    if (!window.confirm("Remove this jumpscare from the published Scare Sheet?")) return;
    setBusy(candidate.id + "remove"); setNotice("");
    const response = await fetch("/api/admin/action", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind: "removeCandidate", id: candidate.id }) });
    const body = await response.json(); setBusy(null);
    if (!response.ok) { setNotice(body.error ?? "Jumpscare could not be removed."); return; }
    setNotice("Jumpscare removed from the published Scare Sheet."); await load();
  }
